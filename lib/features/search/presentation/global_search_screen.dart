...
class _GlobalSearchScreenState extends ConsumerState<GlobalSearchScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final query = normalizeSearch(controller.text);
    ...
    if (query.isNotEmpty) {
      ...
      if (filter == _SearchKind.all || filter == _SearchKind.projects) {
        for (final project in ref.watch(projectsProvider)) {
          final haystack = '${project.title} ${project.description ?? ''}'
              .normalizedForSearch;
      ...
      if (filter == _SearchKind.all || filter == _SearchKind.notes) {
        for (final note in ref.watch(notesProvider)) {
          ...
          final haystack =
              '${note.title} ${note.plainText} ${note.tags.join(' ')}'
                  .normalizedForSearch;
          if (haystack.contains(query)) {
            results.add(
              _SearchItem(
          ...
          if (haystack.contains(query)) {
            results.add(
              _SearchItem(
                ...
                title: person.name,
                subtitle: person.relationship ?? l10n.people,
      ...
      if (filter == _SearchKind.all || filter == _SearchKind.shopping) {
        for (final list in ref.watch(shoppingProvider)) {
          final matches =
              list.title.normalizedForSearch.contains(query) ||
              list.items.any(
                (item) => item.title.normalizedForSearch.contains(query),
              );
          if (matches) {
            results.add(
              _SearchItem(
          ...
          if (haystack.contains(query)) {
            results.add(
              _SearchItem(
                ...
                title: medicine.name,
                subtitle: '${medicine.dosage} • ${medicine.time}',
      ...
      if (filter == _SearchKind.all || filter == _SearchKind.finance) {
        for (final transaction in ref.watch(financeProvider).transactions) {
          final haystack =
              '${transaction.category ?? ''} ${transaction.note ?? ''} ${transaction.amount}'
                  .normalizedForSearch;
          if (haystack.contains(query)) {
            results.add(
              _SearchItem(
                icon: Icons.account_balance_wallet_rounded,
                title: transaction.category ?? l10n.finance,
...
String normalizeSearch(String value) => toEnglishDigits(value)
    .toLowerCase()
    .replaceAll('ي', 'ی')
    .replaceAll('ك', 'ک')
    .replaceAll('\u200c', ' ')
...
extension _SearchNormalize on String {
  String get normalizedForSearch => normalizeSearch(this);
}
