...
void main() {
  test(
    'record sync downloads remote changes and uploads local queue',
    () async {
  ...
  test(
    'same-version edits from different devices are kept as conflict',
    () async {
      final local = SyncEnvelope(
...
class _MemoryStore implements SyncStore {
  ...
  @override
  Future<T> atomic<T>(Future<T> Function() action) => action();
  ...
  Future<void> markUploaded(Iterable<String> identities) async {
    ...
    pending.removeWhere((change) => ids.contains(change.changeId));
  }
...
class _MemoryTransport implements SyncTransport {
  ...
  Future<SyncPullResult> pull({
    ...
    String? cursor,
  }) async => SyncPullResult(changes: remoteChanges, nextCursor: 'cursor-1');
  ...
  Future<SyncPushResult> push({
    ...
    required List<SyncEnvelope> changes,
      ...
      SyncPushResult(accepted: changes.map((item) => item.changeId!).toList());
}
