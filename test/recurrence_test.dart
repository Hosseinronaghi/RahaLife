...
void main(){
  test('habit completion is specific to one day without requiring notifications',(){
    final habit=HomeEntry(id:'h',type:HomeEntryType.habit,title:'Read',dateTime:DateTime(2026,9,15),completedDates:['2026-9-15']);
  ...
  test('weekly recurrence never creates events before its start',(){
    final entry=HomeEntry(id:'w',type:HomeEntryType.affair,title:'Weekly',dateTime:DateTime(2026,9,15),reminder:const ReminderPlan(repeat:ReminderRepeat.weekly));
  ...
  test('Solar Hijri birthdays follow the selected calendar across years',(){
    final entry=HomeEntry(id:'b',type:HomeEntryType.birthday,title:'Birthday',dateTime:Jalali(1403,1,1).toDateTime(),calendar:'jalali');
  ...
  test('monthly day 31 does not overflow into a different month',(){
    final entry=HomeEntry(id:'m',type:HomeEntryType.affair,title:'Monthly',dateTime:DateTime(2026,1,31),reminder:const ReminderPlan(repeat:ReminderRepeat.monthly));
